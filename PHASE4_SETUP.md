...
### 3.3. Telegram ID поставщика для ЛС

**Штатный путь (без SQL):** `/menu` → «Поставщики» → «➕ Добавить поставщика» → ввести имя → выбрать:
- **Переслать сообщение** — переслать боту любое сообщение от поставщика;
- **Ссылка** — бот даст одноразовую ссылку `https://t.me/<bot>?start=<token>` (TTL 24 ч), поставщик открывает её и нажимает Start;
- **Пропустить** — поставщик создаётся без ЛС, работа только через групповой чат.

Привязать/сменить/отвязать ID можно в любой момент в карточке поставщика: **Привязать личку** / **Сменить личку** / **Отвязать личку**. Отвязка не трогает групповой чат.

Технарь больше не пишет `suppliers.telegram_id` вручную.
