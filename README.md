...
- **Готовность 2026-09-09:** публичный `/health` backend `db: ok`; webhook/jobs закрыты 401. Parser снаружи не виден. После `audit/hardening`: webhook идемпотентность коммитит `update_log` *до* обработки (повтор Telegram не дублирует действие; ошибка обработки не ретраится). Каналы прайса требуют parser `0003` + redeploy. Таблица: `СЕРВИСЫ.md` § «Готовность».
